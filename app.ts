...
function turnOff(){  //function to set a timeout for the animation
    return new Promise((resolve)=>{
        setTimeout(resolve,2500) //will turn off after 2.5 sec
    })
}

async function display(){ //function to display the annimation
...
function Addition(numb1:number,numb2:number):void{ //function for addition operation
    console.log(chalk.bgBlue(`${numb1} + ${numb2} = ${numb1+numb2}`)) /*the result will be highlighted with blue
    color*/
}

function Subtraction(numb1:number,numb2:number):void{ //function for subtraction operation
    console.log(chalk.bgBlue(`${numb1} - ${numb2} = ${numb1-numb2}`))
}

function Multiplication (numb1:number,numb2:number):void{ //function for multiplication operation
    console.log(chalk.bgBlue(`${numb1} * ${numb2} = ${numb1*numb2}`))
}

function Division (numb1:number,numb2:number):void{ //function for division operation
    console.log(chalk.bgBlue(`${numb1} / ${numb2} = ${numb1/numb2}`))
}


async function Operation(){ //function to ask user for input and perform operation
    const operation=await inquirer.
    prompt([{
        name:'operator', //variable with name 'operator', which is type number
        type:"list",
        message:'What operation do you want to perform? \n', //question
        choices:['Addition','Subtraction','Multiplication','Division'] //options to choose from
    },
    {
        name:'numb1', //first number
        type:'number',
        message:'Enter first number: '
    },
    {
        name:'numb2', //second number
        type:'number',
...
async function doingAgain(){ //function asking the user if he/she wants to do calculation again
